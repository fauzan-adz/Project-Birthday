// Masonry.tsx - KODE LENGKAP YANG DIOPTIMALKAN UNTUK SCROLL HORIZONTAL
import React, { useEffect, useLayoutEffect, useMemo, useRef, useState } from 'react';
import { gsap } from 'gsap';

// [Hooks useMedia, useMeasure, preloadImages, Interfaces TIDAK BERUBAH]

const useMedia = (queries: string[], values: number[], defaultValue: number): number => {
    // ... (implementasi useMedia)
...
interface Item { id: string; img: string; url: string; height: number; }
interface GridItem extends Item { x: number; y: number; w: number; h: number; }
interface MasonryProps {
    items: Item[]; ease?: string; duration?: number; stagger?: number;
    animateFrom?: 'bottom' | 'top' | 'left' | 'right' | 'center' | 'random';
    scaleOnHover?: boolean; hoverScale?: number; blurToFocus?: boolean;
...
const Masonry: React.FC<MasonryProps> = ({
    ...
    // [Fungsi getInitialPosition - Harus didefinisikan di sini]
    ...
    useLayoutEffect(() => {
        ...
        // -----------------------------------------------------
        // PERBAIKAN: Hitung dan Set Lebar Container Instan (Sebelum Animasi Item)
        // -----------------------------------------------------
        const maxContainerWidth = grid.length > 0 ? Math.max(...grid.map(i => i.x + i.w), 0) + 16 : 0;
        const maxContainerHeight = Math.max(...grid.map(i => i.y + i.h), 0);

        if (maxContainerWidth > width) {
            gsap.set(containerRef.current, { width: maxContainerWidth });
        } else {
            gsap.set(containerRef.current, { width: '100%' });
        }
        
        // Animasi Tinggi (Vertikal) tetap dipertahankan
        ...
        grid.forEach((item, index) => {
            ...
            if (!hasMounted.current) {
                ...
                // Animasi Masuk Awal: Hanya posisi (x, y) dan opacity yang dianimasikan
                gsap.fromTo(
                    selector,
                ...
                // Animasi Update/Resize (Animasi semua properti)
    ...
    // [Fungsi handleMouseEnter dan handleMouseLeave - TIDAK BERUBAH]
